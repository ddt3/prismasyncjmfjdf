...
"""
#pylint: disable=invalid-name,bad-indentation, bare-except
import re
import sys
import time
...
def read_pdf (pdf_file):
  ...
  try:
    ...
    with open(pdf_file, encoding="binary") as file:
      pdf=file.read()
  except :
...
def RemoveQueueEntries (url, **kwargs):
  """Removes all queue entries with the given states or removes a specific queue entry id
  ...
  """
  # First check if kwargs contains QueueEntryId or status
  queue_ids=[]
  if "QueueEntryID" in kwargs:
    QueueEntryID=kwargs["QueueEntryID"]
    queue_ids.append(QueueEntryID)
  elif "status" in kwargs:
    status=kwargs["status"]
    # Request QueueEntryIDs having a specific state
    queue_ids=ReturnQueueEntries(url,status)
  else:
    print("No QueueEntryID or status given")
    return 0
  ...
  if nrjobs:
    ...
    try:
      ...
      delresponse =  requests.post(url=url,data=deletemessage, headers=headers, timeout=30)
...
def CreateMimePackage (jmf_file, jdf_file,pdf_url) :
  ...
  with open(unique_filename, 'w', encoding="UTF-8") as outfile:
    ...
    with open(jdf_file, 'r', encoding="UTF-8") as tempfile:
      ...
      if sendmime :
        jdf_message=re.sub(" URL=\".*?\""," URL=\"cid:part3@PRISMAsync.printer\"",jdf_message)
      else :
